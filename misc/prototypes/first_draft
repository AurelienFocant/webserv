...
int	main()
{
	...
	while (1) {
		...
		for (size_t i = 0; i < pollRequests.size(); i++)
		{
			if (!(pollRequests[i].revents & POLLIN))
				continue;
			else 
				std::cout << "pollin happened" << std::endl;

			if (pollRequests[i].fd == listenSocket)
			{
			...
			else
			{
				...
				if ((send(pollRequests[i].fd, (const void *) response.c_str(), response.length(), MSG_NOSIGNAL)) < 0)
					perror("send: ");
